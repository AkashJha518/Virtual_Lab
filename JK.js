document.addEventListener('DOMContentLoaded', () => {
    ...
    let connectedWires = []; // We will store all wire data here
    let wireCount = 0;
    ...
    function redrawAllWires() {
        ...
        for (const wire of connectedWires) {
            const outEl = document.querySelector(`[data-node="${wire.outNodeId}"]`);
            const inEl = document.querySelector(`[data-node="${wire.inNodeId}"]`);
            
            if (outEl && inEl) {
                drawSingleWire(outEl, inEl, wire.wireId);
            }
        }
    }

    // --- THIS IS THE CORRECTED FUNCTION ---
    function drawSingleWire(outEl, inEl, wireId) {
        const boardRect = circuitBoard.getBoundingClientRect();
        
        // Prevent drawing if the board has no size (e.g., it's hidden)
        ...
        // 1. Get pixel coordinates relative to the board's top-left corner
        const pixelX1 = outRect.left + outRect.width / 2 - boardRect.left;
        const pixelY1 = outRect.top + outRect.height / 2 - boardRect.top;
        const pixelX2 = inRect.left + inRect.width / 2 - boardRect.left;
        const pixelY2 = inRect.top + inRect.height / 2 - boardRect.top;

        // 2. Normalize these pixel coordinates into the 1000x450 viewBox system
        // (pixel_coord / current_board_size) * viewBox_size
        const x1 = (pixelX1 / boardRect.width) * 1000;
        const y1 = (pixelY1 / boardRect.height) * 450;
        const x2 = (pixelX2 / boardRect.width) * 1000;
        const y2 = (pixelY2 / boardRect.height) * 450;

        // 3. Calculate intermediate point in the viewBox coordinate system
        const midX = x1 + (x2 - x1) / 2;

        // 4. Create the wire
        const wire = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        // The pathData now uses the stable viewBox coordinates (e.g., "M 900 150...")
    ...
    function onNodeClick(e) {
        ...
        if (isInput) {
            ...
            const isAlreadyConnected = connectedWires.find(w => w.wireId === wireId);

            if (correctConnections[outputNodeId]?.includes(inputNodeId) && !isAlreadyConnected) {
                // CORRECT
                connectedWires.push({ outNodeId: outputNodeId, inNodeId: inputNodeId, wireId });
                wireCount++;
                redrawAllWires(); 
            ...
            validTargets.forEach(targetId => {
                ...
                const isAlreadyConnected = connectedWires.find(w => w.wireId === wireId);
                if (targetEl && !isAlreadyConnected) {
    ...
    function activateSimulator() {
        ...
        clearAllHints();
        // The ResizeObserver will AUTOMATICALLY call redrawAllWires()
        // when the block/none change causes the layout shift.
    }
    ...
    // --- THIS OBSERVER FIXES THE RESIZING AND LAYOUT SHIFT BUGS ---
    if ('ResizeObserver' in window) {
        const observer = new ResizeObserver(() => {
            redrawAllWires();
        });
        observer.observe(circuitBoard);
    } else {
